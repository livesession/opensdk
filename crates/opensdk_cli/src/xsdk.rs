...
use opensdk_core::emitter::{EmitterFns, OperationDocs};
...
fn sdk_langs() -> Vec<SdkLang> {
    vec![
        SdkLang {
            id: "go",
            emitter: &opensdk_go::EMITTER,
        },
        SdkLang {
            id: "python",
            emitter: &opensdk_python::EMITTER,
        },
        SdkLang {
            id: "typescript",
            emitter: &opensdk_node::EMITTER,
        },
        SdkLang {
            id: "ruby",
            emitter: &opensdk_ruby::EMITTER,
        },
        SdkLang {
            id: "java",
            emitter: &opensdk_java::EMITTER,
        },
        SdkLang {
            id: "csharp",
            emitter: &opensdk_dotnet::EMITTER,
        },
    ]
}
...
pub fn embed_xsdk(raw_doc: &Value, langs: Option<&[String]>) -> Result<EmbedXSdkResult> {
    ...
    let ir = openapi2opensdk::openapi2opensdk(raw_doc, None)
...
pub fn xsdk_command(opts: &XsdkCommandOptions) -> Result<()> {
    ...
    let doc: Value = if json {
        ...
        oas_doc::parse_spec(&content, &opts.spec).map_err(|e| Error::msg(e.to_string()))?
    };
