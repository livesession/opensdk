...
//! `ExampleValue` tree; `example.rs` renders that tree into Java literals, so the
//! Go/Python/Ruby/Java suites exercise identical shapes and can never drift.
//!
//! The `Enum`/`Object` variants additionally carry the source `type_name` — the
//! Java renderer needs it to resolve an enum constant (`PetStatus.AVAILABLE`) and
//! to `new` a decode-only model POJO for a required object-typed field.

use std::collections::{HashMap, HashSet};

use serde_json::Value;

/// A language-neutral example value; `example.rs` renders it to Java syntax.
pub enum ExampleValue {
    ...
    Null,
    Binary,
    Enum { type_name: String, value: Value },
    Const(Value),
    Array(Box<ExampleValue>),
    Map(Box<ExampleValue>),
    // A model POJO renders as `new X()` (decode-only, no builder), so only the
    // type name is retained — nested field values are never emitted.
    Object { type_name: Option<String> },
...
/// One positional path argument (the guard test targets it by position).
pub struct PathArg {
    pub value: ExampleValue,
}
...
fn scalar_example(r: &Value, hint: Option<&str>, realistic: bool) -> ExampleValue {
    ...
    match r.get("scalar").and_then(Value::as_str) {
        ...
        _ => ExampleValue::Str(if realistic {
            opensdk_core::example::realistic_string(&fmt, hint)
        } else {
...
fn ref_example(
    ...
    match named.get("kind").and_then(Value::as_str) {
        ...
        // A nested struct-typed field renders as `new X()` (the model POJO has a
        // public no-arg constructor), so its fields need not be planned.
        _ => ExampleValue::Object {
            type_name: Some(name.to_string()),
        },
    }
}
