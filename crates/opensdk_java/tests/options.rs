...
use opensdk_java::{generate_java, generate_java_with};
...
fn an_empty_bag_is_a_no_op() {
    let base = generate_java(&fixture());
    assert_eq!(generate_java_with(&fixture(), &json!({})), base);
    assert_eq!(generate_java_with(&fixture(), &Value::Null), base);
}

#[test]
fn base_url_reaches_the_output() {
    let files = generate_java_with(&fixture(), &json!({ "baseURL": "https://custom.test/v9" }));
...
fn tests_false_drops_files() {
    let with = generate_java_with(&fixture(), &json!({}));
    let without = generate_java_with(&fixture(), &json!({ "tests": false }));
    assert!(
