//! Does the generated Java SDK actually COMPILE?
//!
...
//! `@xyd-js/opensdk-java` `O2S_JAVA_SMOKE` suite.
//!
//! Skips when javac is absent, so `cargo test --workspace` stays offline. Set
//! `XYD_SMOKE_JAVA=1` to make a missing toolchain a FAILURE instead — that is
//! what CI uses, so the tier cannot silently stop running there.

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

use opensdk_e2e::{compile_smoke, toolchain_available};
use opensdk_java::generate_java;
...
fn compile_case(case: &str) {
    ...
    let dir = std::env::temp_dir().join(format!("xyd_java_smoke_{case}"));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("temp dir");
    write_project(&generate_java(&ir), &dir);

    let result = compile_smoke("java", &dir);
    let _ = std::fs::remove_dir_all(&dir);
    match result {
        Ok(true) => {}
        Ok(false) => panic!("javac vanished mid-test"),
        Err(e) => panic!("[{case}] generated Java does not compile:\n{e}"),
    }
}
...
fn gated_skip() -> bool {
    if toolchain_available("java") {
        return false;
    }
    if std::env::var("XYD_SMOKE_JAVA").as_deref() == Ok("1") {
        panic!("XYD_SMOKE_JAVA=1 but javac is not installed");
    }
    eprintln!("javac absent — compile smoke SKIPPED");
    true
}
