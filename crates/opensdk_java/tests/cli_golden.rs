...
//!   XYD_BLESS=1 cargo test -p opensdk_java --test cli_golden
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_java(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let files = generate_java(&spec);
    assert!(files.contains_key("src/main/java/com/example/plain/Transport.java"));
    assert!(!files.contains_key("src/main/java/com/example/plain/CliTransport.java"));
}
