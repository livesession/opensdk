//! `opensdk_node` — pure Rust port of the `@xyd-js/opensdk-node` emitter
...
pub fn generate_node_with(
    ...
    if opensdk_cli_common::is_cli_spec(spec_json) {
...
pub(crate) fn with_file_header(rel_path: &str, content: String) -> String {
    opensdk_core::header::with_file_header(rel_path, content)
}
...
pub const EMITTER: opensdk_core::emitter::EmitterFns =
    opensdk_core::emitter::EmitterFns::new("node", generate_node)
...
pub fn generate_node_files(
    ...
    opensdk_core::emitter::attach_write_modes("node", generate_node_with(spec, options))
}
