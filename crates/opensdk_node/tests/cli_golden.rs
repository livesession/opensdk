...
//!   XYD_BLESS=1 cargo test -p opensdk_node --test cli_golden

use std::path::{Path, PathBuf};

use opensdk_cli_common::testkit;
use opensdk_node::generate_node;
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_node(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let files = generate_node(&spec);
    assert!(files.contains_key("src/core/request.ts"));
    assert!(!files.contains_key("src/core/command.ts"));
}
