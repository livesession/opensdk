...
recorded byte-for-byte so the Rust port (`crates/opensdk_chain`) can be gated on it.
...
`crates/opensdk_chain/tests/oracle.rs` and `tests/jsonpath_sweep.rs`, which also assert each
