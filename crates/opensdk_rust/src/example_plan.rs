...
//! `ExampleValue` tree; `example.rs` renders that tree into Rust literals, so the
//! Go/Python/Ruby/Rust suites exercise identical shapes and can never drift.

use std::collections::HashSet;

use serde_json::{Map, Value};

/// A language-neutral example value; `example.rs` renders it to Rust syntax.
/// (Enum/Object/Union carry the source type name so Rust can spell typed
/// variants — `PetStatus::Available`, `Type { .. }`, `Type::Variant0(..)`.)
