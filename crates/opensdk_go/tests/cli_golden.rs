...
//!   XYD_BLESS=1 cargo test -p opensdk_go --test cli_golden

use std::path::{Path, PathBuf};

use opensdk_cli_common::testkit;
use opensdk_go::generate_go;
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_go(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let files = generate_go(&spec);
    assert!(files.contains_key("internal/requestconfig/requestconfig.go"));
    assert!(!files.contains_key("internal/runner/runner.go"));
}
