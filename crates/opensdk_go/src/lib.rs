...
fn resolve_options(spec: &Value, options: &Value) -> Options {
    use opensdk_core::emitter::opt_str;
...
// The header text + per-extension comment table live in opensdk_core. The
...
pub(crate) fn with_header(rel: &str, content: String) -> String {
    opensdk_core::header::with_file_header(rel, content)
}
...
pub fn generate_go_with(
    ...
    if opensdk_cli_common::is_cli_spec(spec) {
    ...
    let sdk_behavior = opensdk_core::behavior::resolve_behavior(spec);
    let ctx = GoCtx {
    ...
    if opensdk_core::emitter::emit_tests(options) {
        for (path, content) in
...
pub fn generate_go_usage(
    ...
    example_go::go_usage(
        ...
        &opts.module_path,
        &opts.pkg,
        opensdk_core::emitter::opt_str(options, "baseUrlEnv"),
    )
}
...
pub fn generate_go_type_reference(
    ...
    method: &Value,
    _options: &Value,
) -> opensdk_core::emitter::RenderedTypeReference {
...
/// implements them. See `opensdk_core::emitter` for why this is a data
/// descriptor rather than a trait.
pub const EMITTER: opensdk_core::emitter::EmitterFns =
    opensdk_core::emitter::EmitterFns::new("go", generate_go)
...
) -> std::collections::BTreeMap<String, opensdk_core::emitter::GeneratedFile> {
    opensdk_core::emitter::attach_write_modes("go", generate_go_with(spec, options))
}
