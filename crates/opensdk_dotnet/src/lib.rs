//! opensdk_dotnet — Rust port of the IR→C# emission from
...
pub fn generate_dotnet_with(spec: &Value, options: &Value) -> BTreeMap<String, String> {
    ...
    if !resources.is_empty() && opensdk_core::emitter::emit_tests(options) {
...
pub const EMITTER: opensdk_core::emitter::EmitterFns =
    opensdk_core::emitter::EmitterFns::new("dotnet", generate_dotnet)
...
/// `mergeJson` deep-merges into the user's file. The flat [`generate_dotnet`]
...
pub fn generate_dotnet_files(
    ...
    opensdk_core::emitter::attach_write_modes("dotnet", generate_dotnet_with(spec, options))
}
