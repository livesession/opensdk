...
use opensdk_dotnet::{generate_dotnet, generate_dotnet_with};
...
fn an_empty_bag_is_a_no_op() {
    let base = generate_dotnet(&fixture());
    assert_eq!(generate_dotnet_with(&fixture(), &json!({})), base);
    assert_eq!(generate_dotnet_with(&fixture(), &Value::Null), base);
}

#[test]
fn base_url_reaches_the_output() {
    let files = generate_dotnet_with(&fixture(), &json!({ "baseURL": "https://custom.test/v9" }));
...
fn tests_false_drops_files() {
    let with = generate_dotnet_with(&fixture(), &json!({}));
    let without = generate_dotnet_with(&fixture(), &json!({ "tests": false }));
    assert!(
