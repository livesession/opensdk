...
//!   XYD_BLESS=1 cargo test -p opensdk_dotnet --test cli_golden

use std::path::{Path, PathBuf};

use opensdk_cli_common::testkit;
use opensdk_dotnet::generate_dotnet;
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_dotnet(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let transport = files.get("Transport.cs").expect("Transport.cs emitted");
    assert!(
        transport.contains("class ApiException"),
        "HTTP mode must keep the HTTP transport"
    );
    assert!(
        !transport.contains("class CliException"),
        "CLI transport must not leak into HTTP mode"
    );
    assert!(
        files.contains_key("Models.cs"),
        "HTTP mode always emits Models.cs"
    );
}
