...
use opensdk_diff::diff_ir;
use serde_json::Value;

fn corpus_dir() -> PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../opensdk_core/__fixtures__/diff")
}
...
fn matches_the_typescript_oracle() {
    ...
    let cases = parity_kit::fixture_cases(&corpus);
    ...
    for case_dir in &cases {
        ...
        if !parity_kit::canon::canon_eq(&actual, &parity_kit::read_oracle(case_dir)) {
    ...
    if let Some(first) = failed.first() {
        ...
        let actual = serde_json::to_value(diff_ir(&base, &head)).expect("serialize diff");
        parity_kit::assert_parity(first, &actual);
...
fn oracle_covers_every_change_kind() {
    ...
    for case_dir in parity_kit::fixture_cases(&corpus) {
        let (base, head) = load_pair(&corpus, &case_dir);
