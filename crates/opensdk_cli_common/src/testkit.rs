...
/// `crates/opensdk_cli_common/__fixtures__/cli` — the shared emitter
...
pub fn recording_cli_path() -> PathBuf {
    ...
    PATH.get_or_init(|| {
        // CARGO_MANIFEST_DIR is <repo>/crates/opensdk_cli_common; TWO pops
        ...
        assert!(status.success(), "cargo build recording-cli failed");
