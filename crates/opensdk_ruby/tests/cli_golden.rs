...
//!   XYD_BLESS=1 cargo test -p opensdk_ruby --test cli_golden
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_ruby(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let files = generate_ruby(&spec);
    assert!(files.contains_key("lib/plain/transport.rb"));
    assert!(!files.contains_key("lib/plain/runner.rb"));
}
