...
//! of goldens (see `opensdk_core`'s lib docs). So the loader hands back a
...
///
/// Not re-implemented here: it was already ported (into `opensdk_core`,
/// where the emitters needed it), and one implementation is the point.
pub use opensdk_core::emitter::walk_methods;
...
pub enum LoadSpecError {
    ...
    #[error("HTTP(S) OpenSDK spec sources are not supported by opensdk_config: fetch {0} in the caller and use parse_opensdk_spec")]
