...
//! IN this crate and inputs come from the shared opensdk_cli_common
//! fixtures — never from packages/).
//!
//! Regenerate (then review):
//!   XYD_BLESS=1 cargo test -p opensdk_python --test cli_golden
...
fn cli_golden_all_cases() {
    ...
    for case in &cases {
        ...
        let generated = generate_python(&input);
...
fn http_specs_bypass_cli_mode() {
    ...
    let transport = files
        .get("plain/_transport.py")
        .expect("HTTP mode emits plain/_transport.py");
    assert!(
        transport.contains("urllib.request"),
        "HTTP transport must stay the urllib client"
    );
    assert!(
        !transport.contains("subprocess"),
        "HTTP transport must not pick up the CLI runner"
    );
    assert!(
        files.contains_key("tests/utils.py"),
        "HTTP mode keeps its pytest suite"
    );
}
