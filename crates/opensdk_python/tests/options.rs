...
use opensdk_python::{generate_python, generate_python_with};
...
fn an_empty_bag_is_a_no_op() {
    let base = generate_python(&fixture());
    assert_eq!(generate_python_with(&fixture(), &json!({})), base);
    assert_eq!(generate_python_with(&fixture(), &Value::Null), base);
}

#[test]
fn base_url_reaches_the_output() {
    let files = generate_python_with(&fixture(), &json!({ "baseURL": "https://custom.test/v9" }));
...
fn tests_false_drops_files() {
    let with = generate_python_with(&fixture(), &json!({}));
    let without = generate_python_with(&fixture(), &json!({ "tests": false }));
    assert!(
