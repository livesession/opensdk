//! Env-gated e2e for CLI mode: generate a real Python SDK, run it (python3)
...
//! timeout kill.
//!
//!   XYD_CLI_SMOKE_PYTHON=1 cargo test -p opensdk_python --test cli_smoke
//!
//! Default `cargo test --workspace` skips this (no python3 required).

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};
use std::process::Command;

use opensdk_cli_common::testkit;
use opensdk_python::generate_python;
use serde_json::Value;

fn gated() -> bool {
    std::env::var("XYD_CLI_SMOKE_PYTHON").is_err()
}
...
fn fresh_dir(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("xyd-py-cli-smoke-{}-{name}", std::process::id()));
    if dir.exists() {
...
/// `python3 driver.py` with the recording CLI wired via the spec's envVar.
fn run_driver(project: &Path, env: &[(&str, String)]) -> (String, String, bool) {
    let mut cmd = Command::new("python3");
    cmd.arg("driver.py").current_dir(project);
    for (key, value) in env {
        cmd.env(key, value);
    }
    let out = cmd
        .output()
        .expect("run python driver (is `python3` on PATH?)");
    (
...
const NESTED_DRIVER: &str = r#"import os
import sys

from acme import Client, CliError


def main() -> int:
    kwargs = {}
    ms = os.environ.get("XYD_SMOKE_TIMEOUT_MS")
    if ms:
        kwargs["timeout"] = float(ms)
    client = Client(**kwargs)
    try:
        res = client.chat.completions.create("hello world", model="gpt-4", temperature=0.5)
    except CliError as err:
        print("CLIERR %d %s" % (err.exit_code, err.stderr))
        return 0
...
const BASIC_DRIVER: &str = r#"import sys
...
#[test]
fn python_sdk_spawns_the_cli_with_the_bound_argv() {
    if gated() {
        eprintln!("skipped (set XYD_CLI_SMOKE_PYTHON=1 to run)");
        return;
    }
    ...
    write_project(&generate_python(&shared_input("02.nested")), &project);
    std::fs::write(project.join("driver.py"), NESTED_DRIVER).unwrap();
    ...
    write_project(&generate_python(&shared_input("01.basic")), &project);
    std::fs::write(project.join("driver.py"), BASIC_DRIVER).unwrap();
