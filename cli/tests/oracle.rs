...
fn run_publish_identity(case: &Path) -> Value {
    ...
    let publish: Option<opensdk_config::PublishTarget> = match input.get("publish") {
...
const PETSTORE: &str = "crates/openapi2opensdk/__fixtures__/1.basic/input.json";
...
fn sha256_hex(bytes: &[u8]) -> String {
    ...
    opensdk_framework::sha256_hex(&String::from_utf8_lossy(bytes))
}
