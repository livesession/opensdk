...
//!
//! The chain ENGINE (`opensdk_chain`) owns `resolve_chain` + `process_source`
...
use opensdk_chain::{process_source, resolve_chain, ChainSource};
use opensdk_config::{merge_publish_targets, ChainTarget};
...
fn as_publish(v: &Value) -> Option<opensdk_config::PublishTarget> {
