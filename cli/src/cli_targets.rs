...
use opensdk_config::PublishTarget;
    ...
    fn generate(self, spec: &Value, options: &Map<String, Value>) -> Result<FileMap> {
        ...
        match self {
            CliBackend::Go => {
                ...
                Ok(opencli2go::opencli2go(spec, Some(opts))
                    .into_iter()
                    .map(|(path, content)| {
                        (
                            path,
            ...
            CliBackend::Rust => {
                ...
                Ok(opencli2rust::opencli2rust(spec, Some(opts))
                    .into_iter()
                    .map(|(path, entry)| {
                        let write_mode = match entry.write_mode {
                            opencli2rust::WriteMode::Overwrite => WriteMode::Overwrite,
                            opencli2rust::WriteMode::SkipIfExists => WriteMode::SkipIfExists,
                        };
                        (
                            path,
                            FileEntry {
...
pub fn generate_cli_target(opts: &CliTargetOptions, cwd: &Path) -> Result<()> {
    ...
    let cli_spec = openapi2opencli::openapi2opencli_from_file(&opts.spec, Some(converter_options))
